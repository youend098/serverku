...
expx=$(curl https://raw.githubusercontent.com/youend098/registerku/main/reg | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
Exp1=$(curl https://raw.githubusercontent.com/youend098/registerku/main/reg | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
    sts="${Info}"
else
    sts="${Error}"
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/youend098/registerku/main/reg | grep $ipsaya | awk '{print $2}')
