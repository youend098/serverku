...
echo ""
read -p "🖥️ TEKAN [ENTER] UNTUK MEMULAI PROSES INSTALASI"
echo ""
clear

# MEMERIKSA USER ROOT
if [ "${EUID}" -ne 0 ]; then
    echo "⚠️ ANDA PERLU MENJALANKAN SCRIPT INI SEBAGAI USER ROOT"
    exit 1
fi
...
datediff() {
    d1=$(date -d "$1" +%s)
    d2=$(date -d "$2" +%s)
    echo "KADALUARSA DALAM : $(( (d1 - d2) / 86400 )) HARI"
}
...
function is_root() {
    if [[ 0 == "$UID" ]]; then
        print_install "MEMULAI INSTALASI PENGGUNA USER ROOT"
    else
        print_error "SAAT INI PENGGUNA BUKAN USER ROOT, HARAP BERALIH KE PENGGUNA USER ROOT DAN JALANKAN SCRIPT INI LAGI"
    fi
}
clear
...
function first_setup() {
    ...
    if [[ "$OS_ID" == "ubuntu" ]]; then
        clear
        print_install "SETUP DEPENDENCIES $OS_NAME"
        sleep 1
        sudo apt update -y
        ...
        print_error "OS YANG ANDA GUNAKAN TIDAK DIDUKUNG ($OS_NAME)"
        exit 1
    fi
}
clear
...
EOF
    ...
    function print_ok "KONFIGURASI PAKET SELESAI"
}
...
function udp_mini(){
clear
print_instal "MEMASANG SERVICE LIMIT QUOTA"
sleep 1
...
function ins_restart(){
    clear
    print_install "RESTART SEMUA PAKET"
    sleep 1
    ...
    rm -f /root/cert.pem
    clear
    print_ok "SEMUA PAKET BERHASIL DI RESTART"
    sleep 1
}

function menu(){
    clear
    print_install "MEMASANG SEMUA MENU PAKET"
    sleep 1
...
clear
print_success "SEMUA MENU PAKET"
sleep 1
}
...
function enable_services(){
    ...
    systemctl restart haproxy
    clear
    print_ok "ENABLE SERVICE SELESAI"
    sleep 1
}

function instal(){
clear
print_install "MEMPROSES SEMUA INSTALASI"
sleep 1
first_setup
nginx_install
base_package
...
instal
clear
print_install "HAPUS HYSTORI PROSES INSTALASI"
sleep 1
history -c
rm -rf /root/menu
rm -rf /root/*.zip
